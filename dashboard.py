...
class ScriptDialog(ctk.CTkToplevel):
    ...
    def __init__(self, parent, title="Aggiungi Script", script_data=None):
        ...
        ctk.CTkButton(path_frame, text="Sfoglia", width=50, command=self.browse_file).pack(side="left")
...
class App(ctk.CTk):
    ...
    def __init__(self):
        ...
        # Frame per l'elenco degli script
        self.scrollable_frame = ctk.CTkScrollableFrame(self, label_text="Script Disponibili")
        self.scrollable_frame.grid(row=0, column=0, padx=20, pady=20, sticky="nsew")

        # Pulsante per aggiungere nuovi script
        self.add_button = ctk.CTkButton(self, text="Aggiungi Script", command=self.add_script)
